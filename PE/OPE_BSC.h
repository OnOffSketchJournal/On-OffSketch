...
class OPE_BSC : public Abstract<DATA_TYPE, COUNT_TYPE>{
public:

    OPE_BSC(uint32_t _hash_num, uint32_t _length):
    ...
    void ideal_set(const DATA_TYPE item, const COUNT_TYPE persistency){// TEST UNDER IDEAL CONDITION IN CU SKETCH
        for(uint32_t i = 0;i < hash_num;++i){
            uint32_t pos = this->hash(item, i) % length;
            counters[i][pos] = std::max(counters[i][pos],persistency);
        }
    }
